...
\begin{abstract}
Public mental-health education has helped many people name harmful behavior, seek support, and leave abusive dynamics. This paper does not argue against public education. It argues that open-audience psychological communication can contain an adversarial failure mode: the same detection rules that help victims and bystanders identify harm can also teach motivated actors what to stop showing.

The proposed framework identifies \textit{Detection-Rule Leakage}: the process by which public abuse-detection rules, clinical vocabulary, and relational safety scripts become visible to the same actors those rules are meant to expose. Once detection rules become public, a manipulative actor can update presentation style by replacing overt hostility with approved psychological language. This upgraded presentation is called \textit{Trust Camouflage}. The behavioral function may remain coercive, avoidant, or reality-distorting while the surface language appears calm, therapeutic, accountable, and boundary-focused.

The paper's central safeguard is \textit{function over form}. Public phrase checklists are Goodhartable: once a phrase becomes a target, it can be avoided, copied, or polished. This framework therefore does not ask observers to memorize suspicious phrases. It asks whether a speaker's language preserves repair, evidence, accountability, and proportional consequences over time. The model integrates coercive control, DARVO, gaslighting, concept creep, impression management, framing effects, Goodhart's Law, Campbell's Law, moral-emotional diffusion online, engagement-based ranking systems, and limitations of reinforcement learning from human feedback. It is presented as a falsifiable cognitive-security framework and pre-validation measurement proposal, not as a diagnostic tool.
\end{abstract}

\begin{frameworkbox}{Author Note, Scope, and AI-Use Disclosure}
\term{Author profile:} Michael Zot is an independent researcher focused on cognitive security, digital psychology, adversarial communication, and systems failure analysis. His work develops the MIPU/MAP framework: Minimal Irreversible Predictive Update and Maximal Anticipatory Pattern.

\vspace{0.4em}
\term{AI-use disclosure:} AI assistance was used only for organization, formatting, reference-list scaffolding, copyediting, and sentence-level structuring. No AI system was used as a source of empirical evidence, clinical authority, or theoretical authorship. The conceptual framework, core terms, examples, hypotheses, interpretations, and final claims are the author's responsibility.
...
\begin{enumerate}
    ...
    \item \term{Adversarial literacy:} Manipulative actors learn which behaviors, phrases, tones, and emotional displays have become socially detectable.
\end{enumerate}

The second outcome is the core risk. A manipulative actor does not need to become healthier. The actor only needs to stop producing the signals that observers have been trained to flag. This paper names that failure mode \textit{Detection-Rule Leakage}.
...
Therapy-speak refers to the movement of clinical and therapeutic language into everyday communication. Isern-Mas and Almagro describe therapy-speak as a superficial integration of psychotherapy-derived language into ordinary speech \cite{IsernMas2025}. Haslam's work on concept creep explains how harm-related psychological concepts can expand outward and downward over time \cite{Haslam2016}. This paper focuses on one subset of that drift: adversarial or accountability-blocking use of trusted clinical vocabulary.

\subsection{Goodhart, Campbell, and Detection Metrics}

Goodhart's Law is often summarized as the problem that a measure loses reliability once it becomes a target \cite{Goodhart1975}. Campbell's Law makes a similar point about social indicators: when a metric becomes important for decision-making, it becomes vulnerable to corruption pressure \cite{Campbell1979}. Abuse-detection checklists can be Goodharted. Once the checklist is public, a manipulative actor can optimize against it.
...
\begin{enumerate}
    ...
    \item Trusted psychological language becomes a mask upgrade.
...
\vspace{0.5em}
\term{MIPU: Minimal Irreversible Predictive Update.} In the author's broader MIPU/MAP research program, MIPU was originally coined as Minimal Irreversible Predictive Update. In this manuscript, ``irreversible'' is operationalized as high-friction reversal cost: a substantial increase in the cost of reversing an observer's interpretation, not a claim of literal permanence. For peer-review clarity, the same mechanism may be described as a minimal high-friction predictive update: a minimal trust-bearing cue that raises the cost of reversing an observer's interpretation. The construct is grounded in framing effects \cite{TverskyKahneman1981} and impression management \cite{Goffman1959}.
...
\addlinespace[0.35em]
Trust Camouflage & Trusted language preserves harmful function while improving surface credibility. & Repair, evidence, accountability, consequences, and pattern over time. & Adds the full sequence: public detection rule, adversarial learning, mask upgrade, institutional parser failure, and function-over-form scoring. \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}
    ...
    \item \term{Adversarial reading:} Manipulative actors learn which phrases, tones, and behaviors now create suspicion.
    \item \term{Mask upgrade:} The actor removes obvious red flags and adopts trusted vocabulary.
...
MIPU was originally coined as Minimal Irreversible Predictive Update in the author's broader research program. This manuscript does not replace that term. It states the operational meaning explicitly for peer review: ``irreversible'' denotes high-friction reversal cost, not literal permanence. Once a trusted label enters a conflict, the observer's threshold for challenging the speaker may rise. This is related to broader work on framing and social presentation \cite{TverskyKahneman1981,Goffman1959}, but the MIPU claim is narrower: in accountability conflicts, a small trust-bearing cue can raise the cost of returning attention to the original behavior.
...
% ==========================================
\section{TCFS Coding Protocol v0.2}
...
\begin{enumerate}
    ...
    \item Manipulative actors learn the labels.
    \item New camouflage forms around the labels.
    \item More labels are published to detect the new camouflage.
\end{enumerate}

Without adversarial design, public psychology risks becoming a mask-upgrade manual.
...
\begin{enumerate}
    \item \term{Vignette experiment for H2:} Present participants with identical relational conflicts. Vary only whether the actor uses neutral language, hostile language, or therapeutic language. Measure perceived harm, credibility, repair obligation, blame assignment, and willingness to recommend consequences.
...
\vspace{0.7em}
\term{The New Mask:} ``Old red flags were easier to hear. New masks can sound psychologically literate.''
...
Second, TCFS v0.2 is not a validated instrument. No clinical or institutional threshold is proposed. Future work must test inter-rater reliability, base rates, factor structure, and predictive validity.
...
The checklist caught the old mask. The next task is mapping the upgraded one.
